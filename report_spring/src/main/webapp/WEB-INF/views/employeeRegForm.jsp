...
<head>
	...
	<script>
		$(document).ready(function () {
			...
			inputData("emp_pwd", "${employeeDTO.emp_pwd}");
		...
		function goDept() {
			...
			checkEmployeeRegForm('no');

		}
		//***************************************************************
		// EmployeeRegForm 유효성 체크
		//***************************************************************
		function checkEmployeeRegForm(form) {
			if (form == 'yes') {
				$("[name=empRegCheck]").val('yes');
				...
				console.log($("[name=employeeRegForm]").serialize());

				$.ajax({
					url: "/ware/employeeRegProc.do",
					type: "post",
				...
				if (a != "" && b != "") {
					// alert( $("[name=employeeRegForm]").serialize() );
					$.ajax({
						url: "/ware/employeeRegProc.do",
						type: "post",
						data: $("[name=employeeRegForm]").serialize(),
		...
		<div class="row tm-content-row">
			<div class="mx-auto tm-col-account-settings">
				<div class="tm-bg-primary-dark tm-block tm-block-settings">
					<h2 class="tm-block-title">직원 등록</h2>
					<form method="post" name="employeeRegForm" class="tm-signup-form row" method=POST
						action="/ware/employeeRegForm.do">
						<div class="form-group col-lg-6">
							<label for="emp_name">이름</label>
							<input id="emp_name" name="emp_name" type="text" class="form-control validate emp_name" />
							<div class=emp_name_valid></div>
						</div>
						<div class="form-group col-lg-6">
							<label for="emp_no">사번</label>
							<input id="emp_no" name="emp_no" type="text" class="form-control validate emp_no onlynumber" maxlength="5" />
							<div class=emp_no_valid></div>
						</div>
						<div class="form-group col-lg-6">
							<label for="emp_pwd">암호</label>
							<input id="emp_pwd" name="emp_pwd" type="password" class="form-control validate emp_pwd" />
							<div class=emp_pwd_valid></div>
						</div>
						<div class="form-group col-lg-6">
							<label for="jumin_num">주민등록번호</label>
							<input id="jumin_num" name="jumin_num" type="text" class="form-control validate jumin_num onlynumber" maxlength="13" placeholder=" - 없이 입력바람"/>
							<div class=jumin_num_valid></div>
						</div>
						<div class="form-group col-lg-6">
							<label for="email">이메일</label>
							<input id="email" name="email" type="email" class="form-control validate email" />
						...
						<div class="form-group col-lg-6">
							...
							<input id="phone_num" name="phone_num" type="tel" class="form-control validate phone_num onlynumber" maxlength="15"placeholder=" - 없이 입력바람"/>
						...
						<div class="form-group col-lg-6">
							...
							<input type=button class="btn btn-primary btn-block text-uppercase" value="직원 등록"
								onclick="checkEmployeeRegForm('yes');">
						</div>
						<div class="form-group col-lg-6">
							<label class="tm-hide-sm">&nbsp;</label>
						...
						<input type="hidden" name=empRegCheck>
					</form>
				</div>
			</div>
		</div>
